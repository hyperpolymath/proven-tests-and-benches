...
//
= Proven-Tests: Type-Safe and Beyond Testing Framework Container Templates
:toc: left
:toclevels: 3
:sectnums:
...
IMPORTANT: *Honesty note (2026-07-02):* everything in this directory is
uninitialised template boilerplate — the `{{PLACEHOLDER}}` tokens have never
been replaced and nothing here is wired into this repository's CI. The real
build path is `scripts/ci-build.sh` (no containers involved). Kept for a
future `just container-init` bootstrap; do not mistake it for a working
container setup.
